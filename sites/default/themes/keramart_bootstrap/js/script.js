...
(function ($) {
  ...
  $(window).on("scroll", function () { //верхнее меню появляется только при прокрутке на 100 пк вниз
    var scrollPos = $(window).scrollTop();
    var mql = window.matchMedia('all and (max-width: 768px)'); // только для устройств с разрешением более 768 px
    if (mql.matches) {
      // размер окна 768px или меньше - ничего не делаем
    } else {
      // нет, размер окна более 768px - прячем узкое меню в самом верху 

      if (scrollPos <= 100) {
        $("#navbar").removeClass('navbar_visible');
      } else {
        $("#navbar").addClass('navbar_visible');
      }
    }
  });
