...
function lanpag_form_alter(&$form, &$form_state, $form_id) {
	...
	if (array_key_exists('event', $_REQUEST)) {
		...
		'class' => array('btn-lg', 'btn-warning', 'btn-block1', 'col-md-3', 'col-sm-6', 'col-xs-12'),
		'onclick' => array('yaCounter36494045.reachGoal(\'REGISTR\'); return true;') //Метрика
		);
    }

        if ($form_id == 'webform_client_form_97') {
		//dpm($_REQUEST);
	if (array_key_exists('event', $_REQUEST)) {
        ...
        $form['submitted']['vopros']['#attributes']['placeholder'] = 'Введите вопрос';
        $form['submitted']['telefon']['#attributes']['placeholder'] = 'Введите телефон';
		
        $form['actions']['submit']['#attributes'] = array(
		'class' => array('btn-lg', 'btn-warning', 'btn-block1'),
		'onclick' => array('yaCounter36494045.reachGoal(\'QA\'); return true;') //Метрика
		);
    }
}
...
function lanpag_preprocess_image(&$variables) {
	 //print_r($variables);
    // If this image is of the type 'thumbnail' then assign additional classes to it:
	    if (isset($variables['style_name']) && $variables['style_name'] == '300wide') {
        $variables['attributes']['class'][] = 'wide300';
